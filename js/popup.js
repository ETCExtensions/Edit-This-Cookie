...
function createAccordionList(cks, callback, callbackArguments) {
    ...
    for(var i=0; i<cks.length; i++) {
        currentC = cks[i];
     	...
     	var titleText = (currentC.name) + ((preferences.showDomain) ? " | " + (currentC.domain) : "");
     	var titleElement = $("<h3/>").addClass(currentC.name).append($("<a/>").text(titleText).attr("href", "#"));
...
function setCookieEvents() {
	...
	$('.expiration').scroller({
	    ...
	    maxDate: new Date(2030, now.getMonth(), now.getDate()),
	    dateFormat: "dd/mm/yy",
	    timeFormat: "hh:ii A",
...
function find(pattern) {
    ...
    /*
    for(var i=cookieList.length-1; i>=shiftsMade; i--) {
        var toShift = $("."+cookieList[i].name, "#cookiesList");
        if(pattern != "" && cookieList[i].name.toLowerCase().indexOf(pattern.toLowerCase()) != -1) {
            shiftsMade++;
            $($(toShift).first()).addClass("searchResult");
            toShift.detach();
            $("#cookiesList").prepend(toShift);
            if(cookieList.toTop(i))
                i++;
        }
        else
            $(toShift).removeClass("searchResult");
    }
    */
    /*
    for(var i=cookieList.length-1; i>=shiftsMade; i--) {
    ...
    */
//    $("#cookiesList").accordion('destroy').accordion();
