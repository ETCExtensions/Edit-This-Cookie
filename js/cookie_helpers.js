...
function compareCookies(b,a){
    //console.log("NAME:"+b.name+" VAL:"+b.value+" DOMAIN:"+b.domain+" PATH:"+b.path+" SECURE:"+b.secure+" HTTPONLY:"+b.httpOnly+" EXP:"+b.expiration+" SID:"+b.storeId+"\nNAME:"+a.name+" VAL:"+a.value+" DOMAIN:"+a.domain+" PATH:"+a.path+" SECURE:"+a.secure+" HTTPONLY:"+a.httpOnly+" EXP:"+a.expiration+" SID:"+a.storeId);
    try {
		if(b.name!=a.name)
		    return false;
		if(b.value!=a.value)
		...
		var aHostOnly = (a.hostOnly || a.domain==undefined) ? true : false;
		var bHostOnly = (b.hostOnly || b.domain==undefined) ? true : false;
		if(aHostOnly!=bHostOnly)
		    return false;
		if(!aHostOnly && b.domain!=a.domain )
		    return false;
		
		var aSession = (a.session || a.expirationDate==undefined) ? true : false;
		var bSession = (b.session || b.expirationDate==undefined) ? true : false;
...
var cookiesToString = {
	...
	"netscape" : function(cookies, url) {
		...
		string += "# This file was generated by Edit This Cookie\n";
		if(url != undefined)
			string += "# URL: " + url + "\n";
	...
	"json" : function(cookies, url) {
		...
		for(var i=0; i<cookies.length; i++) {
			cookie = cookies[i];
			string += 	JSON.stringify(cookie, null, 4) + ((i==cookies.length-1) ? "]" : ",\n");
